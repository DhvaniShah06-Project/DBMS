...
--11. Display name of customers with branch whose amount is greater than 4000 and account no is less than 10 from deposit table. 
	SELECT CNAME,BNAME
	FROM DEPOSIT
	WHERE (AMOUNT > 4000) AND (ACTNO < 110)
...
--5. Select all records from BORROW where the amount is greater than 1000 and less than or equal to 7000, and the loan number is between 250 and 600
	SELECT *
	FROM BORROW
	WHERE (AMOUNT >1000) AND (AMOUNT <=7000) AND (LONANO BETWEEN 250 AND 600)
...
--3. Retrieve records from the BORROW table where the loan amount is greater than 3000 and the loan number is not a multiple of 3. 
	SELECT *
	FROM BORROW
	WHERE (AMOUNT > 3000) AND (LONANO % 3 != 0)
