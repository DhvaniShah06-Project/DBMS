...
--5. Count of Products Sold by Region 
	SELECT REGION,COUNT(PRODUCT) AS SOLD_PRODUCTS
	FROM SALES_DATA
	GROUP BY REGION;

--6. Display Sales Amount by Year and Product 
	SELECT YEAR,PRODUCT,SUM(SALES_AMOUNT) AS SALES
	FROM SALES_DATA
...
--9. Display Years with Maximum Sales Exceeding 500 
	SELECT YEAR,MAX(SALES_AMOUNT) AS SALES_AMOUNT
	FROM SALES_DATA
	GROUP BY YEAR
	HAVING MAX(SALES_AMOUNT) > 500;

--10. Display Regions with at Least 3 Distinct Products Sold.
	SELECT REGION, COUNT(DISTINCT PRODUCT) AS 'SOLD_PRODUCT'
	FROM SALES_DATA
	GROUP BY REGION
...
--14. Find the Product with the Highest Sales Across All Regions in 2023. 
	SELECT PRODUCT,MAX(SALES_AMOUNT) AS MAX_SALES
	FROM SALES_DATA
	WHERE YEAR = 2023
	GROUP BY PRODUCT;

--15. Find Regions Where 'TV' Sales in 2023 Were Greater Than 1000.
	SELECT REGION , SUM(SALES_AMOUNT) AS MAX_SALES
	FROM SALES_DATA
...
--1. Display Count of Orders by Year and Region, Sorted by Year and Region
	SELECT YEAR,REGION,COUNT(PRODUCT) AS TOTAL_PRODUCT
	FROM SALES_DATA
	GROUP BY YEAR,REGION
	ORDER BY YEAR,REGION;

--2. Display Regions with Maximum Sales Amount Exceeding 1000 in Any Year, Sorted by Region 
	SELECT REGION,MAX(SALES_AMOUNT) AS MAX_AMOUNT
	FROM SALES_DATA
	GROUP BY REGION
...
--5. Find the Year with the Lowest Total Sales Across All Regions.
	SELECT TOP 1 YEAR, SUM(SALES_AMOUNT) AS SALES
	FROM SALES_DATA
	GROUP BY YEAR
	ORDER BY SUM(SALES_AMOUNT);
...
--2. Display Years with More Than 1 Orders from Each Region 
	SELECT DISTINCT REGION, YEAR , COUNT(PRODUCT) AS COUNT
	FROM SALES_DATA
	GROUP BY YEAR,REGION
